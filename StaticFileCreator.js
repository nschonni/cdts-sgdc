...
output = definition.filterContent( grunt, output, definition, language, targetFileName );
}
...
fs.mkdirSync( distStaticTargetDirName );
...
module.exports = function generateStaticFile( grunt, themeName, definitionFileBasename, getStaticFileDefinition ) {
    ...
    if ( definition[ themeName ] ) {
Object.assign( definition, definition[ themeName ] );
}

    //(apply defaults)
    if ( !definition.fileBaseName ) {
definition.fileBaseName = definitionFileBasename;
}
    if ( !definition.builderFunctionName ) {
definition.builderFunctionName = definition.fileBaseName;
}
    if ( typeof definition.multiLanguageEnabled === "undefined" ) {
definition.multiLanguageEnabled = true;
}
