...
async function breadcrumbHref( theme, lang ) {
    ...
    if ( lang == "en" ) {
 await expect( basicPage.canBreadCrumb ).toHaveHref( "http://www.canada.ca" );
} else {
 await expect( basicPage.canBreadCrumb ).toHaveHref( "http://www.canada.ca" );
}
}
...
async function govCanImgLinkExists( theme, lang, page = "" ) {
    await topPage.open( theme, lang, page );
    if ( lang == "en" ) {
 await expect( topPage.govCanImgLink ).toExist();
} else {
await expect( topPage.govCanImgLinkFR ).toExist();
}
}
...
async function customSearchDefaultValues( theme, lang ) {
    ...
    if ( theme == "gcweb" ) {
 await expect( topPage.customSearchHiddenInput ).not.toExist();
} else {
...
async function searchExistsIntranet( theme, lang ) {
    await basicPage.open( theme, lang );
    if ( lang == "en" ) {
 await expect( topPage.searchAction ).toHaveAttributeContaining( "action", "https://intranet.canada.ca/search-recherche/query-recherche-eng.aspx" );
} else {
 await expect( topPage.searchAction ).toHaveAttributeContaining( "action", "https://intranet.canada.ca/search-recherche/query-recherche-fra.aspx" );
}
}
