...
return;
    ...
    if ( !fs.existsSync( distStaticTargetDirName ) ) {
fs.mkdirSync( distStaticTargetDirName, { recursive: true } );
...
delete targetLocaleData[ keysToRemove[ i ] ];
} //remove obsolete keys if any were found
        ...
        if ( untranslatedKeys.length > 0 ) {
            ...
            if ( throwOnUntranslatedMessages ) {
throw new Error( `Found ${untranslatedKeys.length} untranslated message(s) in [${targetLocale}]` );
}
        }
    } );
};
...
console.warn( `WARNING: Message [xliffId=${sourceNode.parentNode.getAttribute( "id" )}; jsonId=${getSourceMessageLocaleKey( sourceNode.textContent )}] contains sub-elements, it will have to be corrected manually.` );
}
    }
